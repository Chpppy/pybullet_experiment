...
class RobotBase(object):
    ...
    def move_ee(self, action, control_method):
        ...
        for i, joint_id in enumerate(self.arm_controllable_joints):
            p.setJointMotorControl2(self.id, joint_id, p.POSITION_CONTROL, joint_poses[i],
                                    force=self.joints[joint_id].maxForce, maxVelocity=1)
    ...
    def move_ee_in_tool_frame(self, local_delta):
        """
        在末端坐标系中移动机械臂末端
        :param robot_id: 机器人在 pybullet 中的 ID
        :param ee_link_index: 末端执行器(Tool0)的链接索引
        :param local_delta: 末端坐标系下的移动增量 [dx, dy, dz]
        :param move_ee: 你的封装函数，接受 [x, y, z, roll, pitch, yaw]
        """
        # 1. 获取当前末端状态
        link_state = p.getLinkState(self.id, self.eef_id)
        ee_position = np.array(link_state[0])        # 世界坐标系下的位置
        ee_orientation = link_state[1]               # 四元数
        r = R.from_quat(ee_orientation)

        # 2. 将局部增量转换到世界坐标系
        world_delta = r.apply(local_delta)
        target_position = ee_position + world_delta

        # 3. 获取当前姿态（RPY），保持姿态不变（也可以改）
        rpy = r.as_euler('xyz')

        # 4. 调用执行
        self.move_ee(target_position.tolist() + rpy.tolist(), 'end')
    ...
    def get_ee_pose(self):
        ...
        '''
        link_state = p.getLinkState(self.id, self.eef_id)
        ee_position = link_state[0]       # 世界坐标系下的位置
        ee_orientation = link_state[1]               # 四元数
        rpy = p.getEulerFromQuaternion(ee_orientation)
        return list(ee_position + rpy)
    ...
    def arm_IK(self, action):
        ...
        joint_poses = p.calculateInverseKinematics(self.id, self.eef_id, pos, orn,
                                                    self.arm_lower_limits, self.arm_upper_limits, self.arm_joint_ranges, self.arm_rest_poses,
                                                    maxNumIterations=20)
        return list(joint_poses)[:6]
    
    def MoveToDefaultPose(self):
        '''
            机械臂移动到默认位姿
        '''
...
class Panda(RobotBase):
    def __init_robot__(self):
        ...
        self.id = p.loadURDF('./assets/urdf/panda.urdf', self.base_pos, self.base_ori,
...
class UR5Robotiq85(RobotBase):
    def __init_robot__(self):
        self.eef_id = 7
        self.arm_num_dofs = 6
        self.arm_rest_poses = [-0.31157233696170844, -1.6461100604763454, 1.5024460744162536, -1.4282278980035465, 
                               -1.5673747315215967, -0.316856582113316]
        self.id = p.loadURDF(self.current_file_path + '/assets/urdf/ur5_robotiq_85.urdf', self.base_pos, self.base_ori,
                             useFixedBase=True, flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES)
        self.gripper_range = [0, 0.085]
       
       # set friction
        right_inner_finger_pad_id = self.get_link_index_by_name(self.id, 'right_inner_finger_pad')
        left_inner_finger_pad_id = self.get_link_index_by_name(self.id, 'left_inner_finger_pad')
        for link_id in [right_inner_finger_pad_id, left_inner_finger_pad_id]:
            p.changeDynamics(self.id, link_id,
                     lateralFriction=2.0,
                     rollingFriction=0.1,
    ...
    def get_link_index_by_name(self, body_id, target_link_name):
        """
        根据 link 名称返回其对应的 link index(整数)

        :param body_id: 通过 p.loadURDF() 加载得到的物体 ID
        :param target_link_name: URDF 中的 link 名（字符串）
        :return: 如果找到则返回 link index,否则返回 None
        """
...
class UR5Robotiq140(UR5Robotiq85):
    def __init_robot__(self):
        ...
        self.id = p.loadURDF('./assets/urdf/ur5_robotiq_140.urdf', self.base_pos, self.base_ori,
                             useFixedBase=True, flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES)
        self.gripper_range = [0, 0.085]
